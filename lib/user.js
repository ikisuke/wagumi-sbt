...
const userFunction = async () => {
    ...
    tmp = await notion.pages.properties.retrieve({ page_id: page.id, property_id: page.properties.name.id});
    metadataStruct.name = tmp.results[0].title.plain_text;

    tmp = await notion.pages.retrieve({ page_id: page.id });
    metadataStruct.image = tmp.icon.external.url;
    
    tmp = await notion.pages.retrieve({ page_id: page.id });
    metadataStruct.external_url = tmp.url;
